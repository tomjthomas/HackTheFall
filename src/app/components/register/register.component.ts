...
export class RegisterComponent implements OnInit {
  ...
  register(
    ...
    this.serviceMethod.registerUser(obj).subscribe((response) => {
      console.log(response);
      if (response.status === "User Created") {
        sessionStorage.setItem("isLoggedIn", "true");
        sessionStorage.setItem("activeUser", JSON.stringify(response.result));
        this.router.navigate(["/home"]);
      } else {
        console.log("Failed");
        this.message1 = "Registration failed";
      }
    });
  }
}
